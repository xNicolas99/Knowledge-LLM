...
async def scroll_all(collection_name: str) -> List[Dict[str, Any]]:
    """Fetches all payloads from a collection."""
    all_points = []
    offset = None
    try:
        while True:
...
async def recreate_collection(collection_name: str):
    """Deletes and recreates a collection to match current embedding dimension settings."""
    logger.info(f"Recreating Qdrant collection: {collection_name} (dim: {config.EMBEDDING_DIM})")
    try:
